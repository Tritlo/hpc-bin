...
modInfo hpcflags qualDecList tix@(TixModule moduleName _ _ tickCounts _ _ _ _) = do
#else
...
#if __GLASGOW_HASKELL__ >= 907
modReport hpcflags tix@(TixModule moduleName _ _ _ _ _ _ _) = do
#else
...
#if __GLASGOW_HASKELL__ >= 907
          [tix' | tix'@(TixModule m _ _ _ _ _ _ _) <- tickCounts, allowModule hpcflags1 m]
#else
